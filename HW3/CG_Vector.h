...
class CG_Vector: public Krylov_Vector_Base<T>
{
  ...
  public:
    ...
    CG_Vector(Array<T1>& array_input)
        :array(array_input)
    {}

    static Array<T1>& CG_Array(Base& base_array)
    {return dynamic_cast<CG_Vector<T,d>&>(base_array).array;}

    static const Array<T1>& CG_Array(const Base& base_array)
    ...
    {Array<T1>::template Copy<T>(c,CG_Array(bv),array);}

    void Copy(const T c1,const Base& bv1,const Base& bv2)
    {Array<T1>::template Copy<T>(c1,CG_Array(bv1),CG_Array(bv2),array);}
